...
export default function ManageUsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await fetch(`${ENV_CONFIG.BASE_API_URL}${API_ENDPOINTS.ADMIN.USERS.GET_ALL}`, {
        method: 'GET',
        headers: {
  ...
  const handleToggleStatus = async (userId, currentStatus) => {
    ...
    try {
      const endpoint = API_ENDPOINTS.ADMIN.USERS.TOGGLE_STATUS.replace(':id', userId);
  ...
  const handleSaveUser = async () => {
    ...
    setIsSaving(true);
    try {
      const endpoint = API_ENDPOINTS.ADMIN.USERS.UPDATE.replace(':id', selectedUser.id);
      const response = await fetch(`${ENV_CONFIG.BASE_API_URL}${endpoint}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${session.accessToken}`,
            ...
            <div className="grid gap-4 py-4">
              <div className="flex items-center justify-center">
                <Avatar className="h-16 w-16">
                  <AvatarImage src={editFormData.avatar || selectedUser.avatar} alt={editFormData.name} />
                  <AvatarFallback className="text-lg">
                    {editFormData.name ? editFormData.name.split(' ').map(n => n[0]).join('').toUpperCase() : 'U'}
                  </AvatarFallback>
                </Avatar>
              </div>
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleEditFormChange('avatar', e.target.value)}
